                ...
                for (int i = 0; i < SessionFromFile.SVIPT_TaskRepetitions; i++)
                    for (int j = 0; j < SessionFromFile.numberOfTrials; j++)
                    {
                        ...
                        while (k < SessionFromFile.data.Count)
                        {
                            ...
                            InstNormPos = InstNormPos * (3.3/ 4096.0);//5 eh o AD_range que nao foi salvo no txt e 10 eh o num de bits que tambem nao foi salvo, talvez seja necessario adicionar
