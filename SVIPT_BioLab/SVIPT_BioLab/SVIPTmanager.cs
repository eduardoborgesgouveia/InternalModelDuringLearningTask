    ...
    * ... Dados serão armazenados em uma coluna.
    ...
    public class Session
    {
        ...
        public int taskMaxForcePercentage { get; set; } //Percentual de força máxima usado nas TAREFAS desta sessão.
														//Este valor ajusta a curva log.
		public double forcePercentageToChangeCurve { get; set; }
		...
		//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2
		public Queue<int>[] triggerForceSignal;
		public Queue<int>[] triggerForceSignalStart;
		public Queue<int>[,] triggerLocation;
		public int bloco = 0;
		//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2
        ...
        public Session()
        {
            ...
            ID_Volunteer = "";
			baseLineForceVal = 0.634;
            baseLineForceVal = 0.001;
            maxForceVal = 4.0;
            maxForcePercentage = 45;
            minForcePercentage = 35;
            SVIPT_TaskRepetitions = 120;
			forcePercentageToChangeCurve = 0.50;
            numberOfTrials = 1;
            ...
            PosPixels = new Queue<int>[SVIPT_TaskRepetitions,numberOfTrials];
            for (int i = 0; i < SVIPT_TaskRepetitions; i++)
                for (int j = 0; j < numberOfTrials; j++)
                    PosPixels[i, j] = new Queue<int>();


			triggerLocation = new Queue<int>[SVIPT_TaskRepetitions, numberOfTrials];
			for (int i = 0; i < SVIPT_TaskRepetitions; i++)
				for (int j = 0; j < numberOfTrials; j++)
					triggerLocation[i, j] = new Queue<int>();
			//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2
			triggerForceSignal = new Queue<int>[1];
			triggerForceSignal[0] = new Queue<int>();

			triggerForceSignalStart = new Queue<int>[1];
			triggerForceSignalStart[0] = new Queue<int>();
		...
		//Salvar os dados de cada trial em arquivo
		public void saveTrial(string _path, int _trial)
		{
			Thread.CurrentThread.CurrentCulture = CultureInfo.CreateSpecificCulture("en-US");
			StreamWriter trialWriter;
			_path = _path + "\\" + _trial + ".txt";
			//variaveis temporarias para armazenar os valores
			//int[] aux = new int[posPixelsPerTrial[0].Count];

			////copiando o valor das filas para as variaveis e limpando as filas
			//posPixelsPerTrial[0].CopyTo(aux, 0);
			//posPixelsPerTrial[0].Clear();


			using (trialWriter = new StreamWriter(@_path))
			{
				trialWriter.Write("Overshoot/Undershoot/Acerto\r\n");
				trialWriter.Write("Localizacao\r\n");
				//for (int i = 0; i < aux.Length; i++)
				//{
				//	trialWriter.Write(aux[i].ToString()+"\r\n");
				//}
			}
			trialWriter.Close();
		}
		...
		//     public void saveDataFile(string _path)
		//     {
		//         string fn;
		//         Thread.CurrentThread.CurrentCulture = CultureInfo.CreateSpecificCulture("en-US");
		//         StreamWriter writer;
		...
		//		writer.Write("\r\nTrigger de alvo atingido para sinal de força");
		//		while (triggerForceSignal[0].Count>0)
		//		{
		//			writer.Write("\r\n" + triggerForceSignal[0].Dequeue().ToString());
		//		}

		//		writer.Write("\r\nTrigger quando alvo volta para home");
		//		while (triggerForceSignalStart[0].Count>0)
		//		{
		//			writer.Write("\r\n" + triggerForceSignalStart[0].Dequeue().ToString());
		//		}
		...
		//                                 writer.Write(i.ToString() + "\t" + auxili.ToString("D"));
		//                             }
		...
		public void saveDataFile(string _path)
		{
			...
			if (MessageBox.Show("Deseja salvar os dados?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
				...
				{
					string exePath = Application.ExecutablePath; //diretório do executável
					fn = Path.GetDirectoryName(exePath) + "\\" + ".txt";
				}
				using (writer = new StreamWriter(@fn))

				{
					...
					writer.Close();
					string[] aux = fn.Split('.');
					writer = new StreamWriter(aux[0] + "_forceSignal.txt");
					...
					writer = new StreamWriter(aux[0] + "_triggerAlvoAtingido.txt");
					while (triggerForceSignal[0].Count > 0)
					{
						writer.Write("\r\n" + triggerForceSignal[0].Dequeue().ToString());
					}
					...
					writer = new StreamWriter(aux[0] + "_triggerAlvoAtHome.txt");
					//writer.Write("\r\nTrigger quando alvo volta para home");
					while (triggerForceSignalStart[0].Count > 0)
					{
						writer.Write("\r\n" + triggerForceSignalStart[0].Dequeue().ToString());
					...
					writer = new StreamWriter(aux[0] + "_triggerLocation.txt");
					//writer.Write("\r\nTrigger quando alvo volta para home");
					int auxili2 = 0;
					for (int i = 0; i < SVIPT_TaskRepetitions; i++)
						for (int j = 0; j < numberOfTrials; j++)
							while (triggerLocation[i, j].Count > 0)
							{// loop over all data values
								writer.Write("\r\n");
								auxili2 = triggerLocation[i, j].Dequeue();
								writer.Write(i.ToString() + "\t" + auxili2.ToString("D"));
							}
					//Close writer
					writer.Close();
				
					writer = new StreamWriter(aux[0] + "_Epoch40.txt");
					//Serie dos numsamples e epoch40
					writer.Write("\r\n" + "NumSamples tab Epoch40");
					// loop over all data values
					foreach (string val in EpochQueue)
					...
					writer = new StreamWriter(aux[0] + "_location.txt");
					//writer.Write("\r\n" + "Localizacao:");
					int auxili = 0;
					for (int i = 0; i < SVIPT_TaskRepetitions; i++)
						for (int j = 0; j < numberOfTrials; j++)
					...
					writer = new StreamWriter(aux[0] + "_resultsAcuracia.txt");
					//Sequencia informando se houve undershoot ou overshoot nas trials.
					//writer.Write(sviptFileConstant.SessionAccuracy);
					for (int i = 0; i < SVIPT_TaskRepetitions; i++)
						for (int j = 0; j < numberOfTrials; j++)
					...
					writer = new StreamWriter(aux[0] + "_resultsErrors.txt");
		...
		public void saveDataFile()
		{
			...
			if (MessageBox.Show("Deseja salvar os dados?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
				...
				string fnConfig = "\\";
				fn = fnConfig + this.ID_Volunteer + "_" + this.id + ".txt";
				if(fn.Equals(fnConfig + this.ID_Volunteer + "_" + this.id + ".txt"))
				{
					string exePath = Application.ExecutablePath; //diretório do executável
					fn = Path.GetDirectoryName(exePath) + "\\"+ ".txt";
				}
				using (writer = new StreamWriter(@fn))
				{
					...
					writer = new StreamWriter(aux[0] + "_locationErrors.txt");
					targetId = 1;
					//writer.Write("\r\n" + sviptFileConstant.TrialErrors);
					foreach (double val in error)
					{// loop over all data values
